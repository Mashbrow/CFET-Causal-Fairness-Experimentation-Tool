...
def ges_updated(X: ndarray, score_func: str = 'local_score_BIC', maxP: Optional[float] = None,
        parameters: Optional[Dict[str, Any]] = None, node_names: Optional[List[str]] =None) -> Dict[str, Any]:
    """
    ...
    if node_names == None:
        node_names = [("x%d" % i) for i in range(1,N+1)]
...
#Class isn't working as expected as there is an instance class test in the 
# process of generating a graph using BackgroundKnowledge in PC
class BackgroundKnowledge_updated(BackgroundKnowledge):

    def not_parent(self, node, nodes):
        for _node in nodes:
            self.add_forbidden_by_node(node,_node)
    
    def not_child(self, node, nodes):
        for _node in nodes:
            self.add_forbidden_by_node(_node, node)
    

def not_parent(node, bk, nodes):
    for _node in nodes:
        bk.add_forbidden_by_node(node,_node)
    return bk

def not_child(node, bk, nodes):
    for _node in nodes:
        bk.add_forbidden_by_node(_node, node)
    return bk

#Display only skeleton
def to_skeleton(pyd_graph):
    for edge in pyd_graph.get_edges():
        edge.set_dir("none")

def get_descendants(source_node, graph):
    descendants = []
    for node in graph.nodes:
        if graph.is_descendant_of(node, source_node):
